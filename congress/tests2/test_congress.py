...
class TestCongress(base.SqlTestCase):
    ...
    def setUp(self):
        ...
        self.services = harness.create2(helper.root_path(), None)
    ...
    def test_policy(self):
        ...
        self.create_policy('data')
        self.insert_rule('p(x) :- data:fake_table(x)', 'alpha')
        data = self.create_datasource('fake')
        data.state = {'fake_table': set([(1,), (2,)])}
        data.poll()
        helper.retry_check_function_return_value(
            lambda: self.query('p(x)', 'alpha'), 'p(1) p(2)')
    ...
    def create_fake_datasource(self, name):
        return self.api['api-datasource'].add_item(
            {'name': name, 'driver': 'fake', 'config': {}})
