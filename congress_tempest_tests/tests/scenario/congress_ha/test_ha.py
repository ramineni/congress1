...
class TestHA(manager_congress.ScenarioPolicyBase):
    ...
    def setUp(self):
        ...
        self.servers = []
        self.replicas = {}
        self.services_client = self.admin_manager.services_v2_client
        self.endpoints_client = self.admin_manager.endpoints_v2_client
    ...
    def start_replica(self, port_num):
        ...
        # Insert after [DEFAULT] if the parameter is not already present
        index = conf.find('[DEFAULT]') + len('[DEFAULT]\n')
        if conf.find('# bind_port = 1789') == -1:
            conf = conf[:index] + 'bind_port = %d\n' % port_num + conf[index:]
        else:
            conf = conf.replace('# bind_port = 1789',
                                'bind_port = %d\n' % port_num)

        if conf.find('# datasource_sync_period = 60') == -1:
            conf = conf[:index] + 'datasource_sync_period = 5\n' + conf[index:]
        else:
