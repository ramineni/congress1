...
class CreateDatasource(forms.SelfHandlingForm):
    name = forms.CharField(max_length=255, label=_("Datasource Name"),
                           help_text='Name of the datasource')
    driver = forms.ThemableChoiceField(label=_("Driver"),
                                       help_text='Datasource driver' )
    description = forms.CharField(label=_("Description"), required=False,
                                  help_text='Datasource Description' )
    ...
    password = forms.CharField(widget=forms.PasswordInput(render_value=False),
                               label=_('Password'))
    tenant_name = forms.CharField(max_length=255, label=_("Tenant Name"))
    ...
    poll_time = forms.IntegerField(label=_("Poll Interval (in seconds)"),
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            msg = _('Created Datasource "%s"') % datasource_name
            LOG.info(msg)
            messages.success(request, msg)
        except Exception as e:
            msg_args = {'datasource_name': datasource_name, 'error': str(e)}
            msg = _('Failed to create datasource "%(datasource_name)s": '
                    '%(error)s') % msg_args
            LOG.error(msg)
